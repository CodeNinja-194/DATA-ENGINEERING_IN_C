...
int main()
{
    ...
    int count[11]={30,40,40,20,5,3,3,10,4,4,6}, i;

    float n=0;
    for(i=0;i<11;i++)
    n=n+count[i];

float  p_c1, p_c2, c1, c2;
for(i=0;i<11;i++)
{
...
printf("\nProbability that (status = senior) = %f\n",p_c1);
printf("Probability that (status = junior) = %f\n\n",p_c2);

float c_x11=0, c_x12=0, c_x21=0, c_x22=0, c_x31=0, c_x32=0;

for(i=0;i<11;i++)
{
    if(strcmp(status[i],"senior")==0)
    {
        if(strcmp(dept[i],"systems")==0)
        c_x11=c_x11+count[i];
        if(strcmp(age[i],"26-30")==0)
        c_x21=c_x21+count[i];
        if(strcmp(salary[i],"46-50")==0)
        c_x31=c_x31+count[i];
    }
    else
    {
